...
class ChessboardComponent extends React.Component<ChessboardComponentProps, {}> {
    public render(): React.ReactNode {
        ...
        for (let i = 0; i < this.props.chessboard.rows; ++i) {
            const cells: JSX.Element[] = [];

            for (let j = 0; j < this.props.chessboard.cols; ++j) {
                cells.push((
                    <td className={`td-${(i ^ j) % 2 ? "black" : "white"}`}>
                        <img src={`data:image/jpeg;base64,${base64}`} alt={"Piece"}/>
                    </td>
                ));
            }
